...
namespace Video_ClubDB
{
    class Reserva
    {
        ...
        public void MostrarPel(List<Pelicula> peli, int r)
        {
            ...
            if (reader.Read())
            {
                Console.WriteLine($"{reader["Sinopsis"]} {reader["EdadRecomendada"]} {reader["Estado"]}");
            }
            connection.Close();
        }
        ...
        public void GenerarDevolucion(List<Pelicula> peli, int r, string usuario)
        {
            ...
            if (difference > 3)
            {
                ...
                Console.WriteLine($"Se ha superado el limite de días fecha alquiler {fecha1} y fecha devolucion {fecha2}");
